...
var gutil__default = 'default' in gutil ? gutil['default'] : gutil;
var posthtml = require('posthtml');
posthtml = 'default' in posthtml ? posthtml['default'] : posthtml;
var renderHTML = require('posthtml-render');
renderHTML = 'default' in renderHTML ? renderHTML['default'] : renderHTML;
var fs = require('fs');
var fs__default = 'default' in fs ? fs['default'] : fs;
var memoize = require('lodash.memoize');
memoize = 'default' in memoize ? memoize['default'] : memoize;
var postcss = require('postcss');
postcss = 'default' in postcss ? postcss['default'] : postcss;
var prefix = require('postcss-prefix-selector');
prefix = 'default' in prefix ? prefix['default'] : prefix;
...
function ComponentFetcher(_ref) {
  ...
  var readdir = memoize(fs__default.readdirSync);
...
function scope$1(id, nodes, data) {
  if (nodes.javascript) {
    var template = fs__default.readFileSync(path.resolve(__filename, "js_template.js"), "utf8");
...
function cloneFile(file, extension, contents) {
  var newFile = new gutil__default.File({
    cwd: file.cwd,
    base: file.base,
    path: file.path,
...
function index () {
  ...
  return through.obj(function (file, encoding, cb) {
    try {
      ...
      this.emit("error", new gutil.PluginError(name, error));
      return cb();
    }
  });
}
